...
class TestAccidentController:
    ...
    def test_delete_accident(self, db_session, test_client, accident_data, source_data):
        db_session.add(source_data)
        db_session.commit()
        db_session.refresh(source_data)
        ...
        response = test_client.delete(f'/api/accident/{accident_data.id}')
    ...
    def test_delete_accident_when_accident_does_not_exist(self, test_client):
        response = test_client.delete('/api/accident/1')
        assert response.status_code == 404