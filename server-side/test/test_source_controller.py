...
class TestSourceController:
    def test_upload_source_video(self, db_session, test_client, authenticated_user_data):
    ...
    def test_upload_source_video_when_video_not_provided(self, db_session, test_client, authenticated_user_data):
        response = test_client.post('/api/media', data={'title': 'VideoSource',
                                                        'description': 'VideoSource description',
                                                        'source_type': SourceType.VIDEO.value},
                                    headers={'Authorization': 'Bearer ' + authenticated_user_data[1]})
    ...
    def test_upload_source_stream(self, db_session, test_client, authenticated_user_data):
    ...
    def test_upload_source_stream_when_stream_not_provided(self, db_session, test_client, authenticated_user_data):
        response = test_client.post('/api/media', data={'title': 'StreamSource',
                                                        'description': 'StreamSource description',
                                                        'source_type': SourceType.VIDEO.value},
                                    headers={'Authorization': 'Bearer ' + authenticated_user_data[1]})
    ...
    def test_get_live_sources(self, test_client,
        ...
        valid_ids = [uploaded_video_source_status_processing['id'],
                     uploaded_stream_source_status_processing['id']
                     ]
    ...
    def test_start_inference_task_for_video(self, test_client, db_session, uploaded_video_source, authenticated_user_data):
        response = test_client.get(f'/api/media/source/inference/{uploaded_video_source.id}', headers={'Authorization': 'Bearer ' + authenticated_user_data[1]})
        assert response.status_code == 200
        assert db_session.query(Source).first().status == SourceStatus.PROCESSING

    def test_start_inference_task_for_stream(self, test_client, db_session, uploaded_stream_source, authenticated_user_data):
        response = test_client.get(f'/api/media/source/inference/{uploaded_stream_source.id}', headers={'Authorization': 'Bearer ' + authenticated_user_data[1]})
        assert response.status_code == 200
        assert db_session.query(Source).first().status == SourceStatus.PROCESSING

    def test_start_inference_task_when_source_does_not_exist(self, test_client, authenticated_user_data):
        response = test_client.get('/api/media/source/inference/1', headers={'Authorization': 'Bearer ' + authenticated_user_data[1]})
        assert response.status_code == 404

    def test_start_inference_task_when_video_file_does_not_exist(self, test_client, db_session, uploaded_video_source, authenticated_user_data):
    ...
    def test_delete_source_video(self, test_client, uploaded_video_source, authenticated_user_data):
        response = test_client.delete(f'/api/media/source/{uploaded_video_source.id}', headers={'Authorization': 'Bearer ' + authenticated_user_data[1]})
        assert response.status_code == 200
        assert not file_exists(uploaded_video_source.file_path)
        response = test_client.get(f'/api/media/source/{uploaded_video_source.id}')
        assert response.status_code == 404

    def test_delete_source_stream(self, test_client, uploaded_stream_source, authenticated_user_data):
        response = test_client.delete(f'/api/media/source/{uploaded_stream_source.id}', headers={'Authorization': 'Bearer ' + authenticated_user_data[1]})
        assert response.status_code == 200
        response = test_client.get(f'/api/media/source/{uploaded_stream_source.id}')
        assert response.status_code == 404

    def test_delete_source_video_when_source_does_not_exist(self, test_client, authenticated_user_data):
        response = test_client.delete('/api/media/source/1', headers={'Authorization': 'Bearer ' + authenticated_user_data[1]})
        assert response.status_code == 404
    ...
    def test_delete_source_video_with_accidents(self, test_client, uploaded_video_source_with_accidents,
        ...
        response = test_client.delete(f'/api/media/source/{source_id}', headers={'Authorization': 'Bearer ' + authenticated_user_data[1]})
        assert response.status_code == 200
    ...
    def test_terminate_live_stream_when_source_video(self, test_client, db_session,
        ...
        response = test_client.put(f'/api/media/source/stream/{source_id}', headers={'Authorization': 'Bearer ' + authenticated_user_data[1]})
        assert response.status_code == 200

        source = db_session.query(Source).filter(Source.id == source_id).first()
        assert source.status == SourceStatus.TERMINATED

    def test_terminate_live_stream_when_source_does_not_exist(self, test_client, authenticated_user_data):
        response = test_client.put('/api/media/source/stream/1', headers={'Authorization': 'Bearer ' + authenticated_user_data[1]})
        assert response.status_code == 404
